...
class FeatureStorage:
    def __init__(self, max_features=2):
        ...
        self.features = []
        self.path = ''
        self.selfattention = SelfAttention(384, 8 ).to("cuda")
    ...
    def store_feature(self, feature):
        ...
        if len(self.features) > self.max_features:
            del self.features[0]
    # @classmethod
    def get_feature(self, feature_matrix, current_feat):
        self.features.append(feature_matrix.clone())
        self.features.append(current_feat.clone())
...
class TIF(nn.Module):
    def __init__(self,   dim_in=384, hidden_dim=768, dim_out=384):
        super().__init__()
    ...
    def _forward_temporal_attn(self, feature_matrix, current_feat):
        ...
        embed2 = self.temporal_attn(
                embed.transpose(0, 1),                  # (num_track, dim) to (1, num_track, dim)
    ...
    def forward(self, feature_matrix, current_feat):
        feature_matrix,current_feat= featureStorage.get_feature(feature_matrix, current_feat)
