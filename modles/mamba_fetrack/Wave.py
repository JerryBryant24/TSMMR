...
def main():
    # 设置模型参数
    dim = 3  # 特征维度，可以根据需求调整
    wavelet = 'haar'  # 使用的 wavelet，可以选择其他，如 'db1', 'db2' 等
    use_sa = False  # 是否使用空间自注意力
    use_ca = False  # 是否使用通道自注意力

    # 创建一个随机的输入张量，形状为 (batch_size, channels, height, width)
    # 假设我们测试的是一个 64x64 的图像，通道数为 3 (例如 RGB 图像)
    input_tensor = torch.randn(1, 3, 64, 64)  # 单一批次

    # 初始化 LWN 模型
    lwn = LWN(dim=dim, wavelet=wavelet, use_sa=use_sa, use_ca=use_ca)

    out_tensor1 = lwn(input_tensor)
