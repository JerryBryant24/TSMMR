...
from lib.models.mamba_fetrack.Other_Search import Block_TI
from lib.models.mamba_fetrack.Temporal_information_fusion import Cross_layer
from lib.models.mamba_fetrack.Temporal import WaveDownampler
...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        self.blocks_TI = Block_TI(dim=384, num_heads=8)

        self.cross_layer = Cross_layer(hidden_dim=384)

        self.waveDownampler = WaveDownampler(in_channels=384)
...
##################  无人机2搜索区域处理 开始 ##########################
        ...
        Ano_TI, Keep = self.blocks_TI(Ano_TI, img_tokens, text_tokens)

        text_feat, img_feat = torch.split(Ano_TI, [text_tokens, Keep.shape[-1]], dim=1)

##################  无人机2搜索区域处理  结束 ##########################
        # Fu_t = self.cross_layer(Pre_feature, Search_1)
        #
        # Search_1 = Fu_t + Search_1

        _, z = self.waveDownampler(z)

        _, z2 = self.waveDownampler(z2)
