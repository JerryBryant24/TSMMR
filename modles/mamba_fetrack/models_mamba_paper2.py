...
import cv2
from lib.models.mamba_fetrack.Wave import LWN
import numpy as np
...
class PMSFA(nn.Module):
    def __init__(self, inc=3) -> None:
        ...
        self.wave = LWN(dim=4, wavelet='haar', use_sa=False, use_ca=False)

    def forward(self, x):
        x = self.conv(x)
        conv1_out = self.conv1(x)
        conv1_out_1, conv1_out_2 = conv1_out.chunk(2, dim=1)
        conv1_out_2 = self.wave(conv1_out_2)
...
#####  20241112 LXQ  时序图像融合 #######################
    ...
    def forward_features(self, z, x, z2, nlp_word, inference_params=None, if_random_cls_token_position=False,
        ...
        nlp_word = clip.tokenize(nlp_word).to('cuda')
        css = self.clip.encode_text(nlp_word)
        z= self.PMSFA(z)
        z2= self.PMSFA(z2)
